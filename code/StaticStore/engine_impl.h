...
private:
	...
	IndexEntry AllocateArray(const void* data, uint64 size) {
		...
		memcpy((char*)_file.GetMapViewAddress() + offset, data, size);
		return { offset, size };
	}
public:
...
public:
	...
	virtual void SetMetadata(const void* metadata, uint64 metadata_size) override {
		if (IsMetadataSet()) { throw std::invalid_argument("set metadata again"); }
...
public:
	virtual const ArrayIndex CreateArray(const void* data, uint64 size) override {
		if (IsMetadataSet()) { throw std::invalid_argument("create array after setting metadata"); }
		ArrayIndex index = _index_entry_table.size();
		_index_entry_table.push_back(AllocateArray(data, size));
		return index;
	}
	virtual std::pair<const void*, uint64> GetArrayData(ArrayIndex index) const override {
		if (index.value >= _index_entry_table.size()) { throw std::invalid_argument("invalid array index"); }
		IndexEntry entry = _index_entry_table[index.value];
